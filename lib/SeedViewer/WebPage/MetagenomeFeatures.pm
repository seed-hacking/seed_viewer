package SeedViewer::WebPage::MetagenomeFeatures;

# $Id: MetagenomeFeatures.pm,v 1.2 2008-06-23 16:27:47 paczian Exp $
...
use SeedViewer::SeedViewer qw( get_menu_metagenome is_public_metagenome );
...
sub output {
  ...
  my $ss_hash = {};
  map { unless(exists($ss_hash->{$_->[2]})) { $ss_hash->{$_->[2]} = {} }; my $sn = $_->[0]; $sn =~ s/_/ /g; $ss_hash->{$_->[2]}->{$_->[0]} = "<a href='?page=Subsystems&subsystem=".$_->[0]."'>".$sn."</a>" } @$subsystem_info;
  ...
  # fill the table data
  my @table_data = map { [ "<a href='?page=Annotation&feature=".$_->[0]."'>".$_->[0]."</a>", 
			   $_->[1],
			   "<a href='?page=MetagenomeSequence&metagenome=".$org_id."&sequence=".$_->[2]."'>".$_->[2]."</a>",  
			   $_->[3], $_->[4], $_->[5], 
			   $_->[7],
			   $_->[6] || '', 
