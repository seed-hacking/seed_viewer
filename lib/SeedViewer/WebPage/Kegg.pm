package SeedViewer::WebPage::Kegg;
...
sub output {
	...
	&get_menu_metagenome($self->application->menu, $genome);
    } else {
	&get_menu_organism($self->application->menu, $genome);
    }
    ...
    my $num_to_name = &map_num_to_name($application, $base_path, @bottom_maps, @mid_maps);
    ...
    foreach my $map ( $top_map, 
		      (sort {$num_to_name->{$a} cmp $num_to_name->{$b}} @mid_maps),
		      (sort {$num_to_name->{$a} cmp $num_to_name->{$b}} @bottom_maps) )
    {
	...
	foreach my $org ( $genome, @comparison_organisms )
	{
		...
		{
		    my $rast    = $application->data_handle('RAST');
		    my $jobs    = $rast->Job->get_objects( {genome_id => $org} );
		    my $jobnum  = $jobs->[0]{id};
		    my $job_dir = $FIG_Config::rast_jobs || $FIG_Config::fortyeight_jobs;
		...
		{
		    my %seen;
		    foreach my $ec ( $func =~ /EC\:*\s*(\d+\.(?:\d+|-)\.(?:\d+|-)\.(?:\d+|-))/gi )
		    {
			next if $seen{$ec}++;
	...
	foreach my $ec ( keys %$ec_coords )
	{
		...
		foreach my $location ( @{ $ec_coords->{$ec} } )
		{
			...
			else
			{
			    ...
			    my($frag_num, $beg, $end) = split('_', $id);
 			    $info = "<table><tr><th>$ec</th></tr><tr><td>$frag_num</td></tr></table>";
			    $link = "?page=MetagenomeSequence&metagenome=$genome&sequence=$frag_num&subseq_beg=$beg&subseq_end=$end";
			...
			if ( $ec_of_org->{$genome}{$ec}[0] =~ /^fig/ )
			{
			    ...
			    $link = "?page=FeatureList&title=$title&feature=" . join('&feature=', @{ $ec_of_org->{$genome}{$ec} });
			}
			else
			{
			    # mg-rast fragments -- contig_beg_end
			    @id_numbers = map {/^(\d+)/; $1} @{ $ec_of_org->{$genome}{$ec} };
			    $link = "?page=FeatureList&title=$title&metagenome=$genome&feature=" . join('&feature=', @{ $ec_of_org->{$genome}{$ec} });
			}

			my $n_features = scalar @{ $ec_of_org->{$genome}{$ec} };
			my $id_list = join(', ', sort {$a <=> $b} @id_numbers);
    ...
    my $form = $self->start_form('comp_form') .
	       ...
	       qq(<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>) .
	       qq(<th>Select upto 4 organisms for comparison:</th></tr>\n<tr><td>) .
    ...
    my $kegg_map_image = "<img src='$FIG_Config::cgi_url/kegg_map_image.cgi?map=$kegg_map' id='kegg_map' onload='position_divs(document.getElementById(\"kegg_map\"));'>";
    my $html = $headline . $form . $ec_table . $kegg_map_image . $hover->output() . $divs;
    return $html;
}
...
sub all_genome_list {
    ...
    if ( ref $rast ) {
	$public_metagenomes = &get_public_metagenomes($application->dbmaster, $rast);

	foreach my $rec ( @$public_metagenomes ) {
	    $org_seen{$rec->[0]} = 1;
	}
    }
...
sub map_num_to_name {
    my($application, $base_path, @ids) = @_;
    my $line;
    # Insure every map number has a name, to avoid massive errors in the error log.
    my %num_to_name = map { $_ => '' } @ids;
    Trace("Looking for map file in parent directory of $base_path.") if T(3);
    my $fname = "$base_path/../map_title.tab";
    if (open(TAB, "<$fname")) {
      while ( $line = <TAB> ) 
      {
	  chomp $line;
      ...
      Trace("Open failed for KEGG map file $fname: $!") if T(1);
    }
    return \%num_to_name;
}
...
sub genome_system {
    ...
    Trace("System for genome $org is $system.") if T(3);
