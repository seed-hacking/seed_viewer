package SeedViewer::WebPage::MetagenomeBlastRun;
...
sub output {
    ...
    if ( $metagenome_id ) { 
	my $job;
	eval { $job = $self->app->data_handle('RAST')->Job->init({ genome_id => $metagenome_id }); };
	unless ($job) {
    ...
    # set up the menu
    &get_menu_metagenome($self->application->menu, $metagenome_id);
...
sub run_blast {
    ...
    my $fig     = $self->fig();
    my $org_dir = $fig->organism_directory($metagenome_id);
    ...
    if ( ! (-d $db_path and -e "$db_path/$db_file") ) {
      ...
      return "An error occurred while trying to blast against the $metagenome_name ($metagenome_id)";
    }
    
    # having trouble with formatdb -- not permitted to run formatdb for SEED organisms
    # run formatdb if necessary
    $self->run_formatdb_if_needed($db_path, $db_file);
...
sub add_links {
    ...
    $output =~ s/(\d+)(\s+<a href\s*=\s*\#\d+>)/<a href=\"\?page=MetagenomeSequence&metagenome=$metagenome_id&sequence=$1\" target=_blank>$1<\/a>$2/sg;
    $output =~ s/(><a\s+name\s*=\s*\d+><\/a>)(\d+)/$1<a href=\"\?page=MetagenomeSequence&metagenome=$metagenome_id&sequence=$2\" target=_blank>$2<\/a>/sg;
...
sub run_formatdb_if_needed {
    ...
    {
	my $cmd  = "$FIG_Config::ext_bin/formatdb -i $db_path/$db_file -n $db_path/$db_file -p F";
	my $fig  = $self->fig();
	$fig->run($cmd);
    }
}
