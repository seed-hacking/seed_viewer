package SeedViewer::WebPage::SV_ServRole;
...
sub page_content
{
    ...
    my $env = {fig => $fig,
           ...
           user => $user_name,
           url => $my_url,
           hidden_form_var => "<input type='hidden' name='page' value='SV_ServRole'>",
...
if ($cgi->param('kb'))
{
    ...
    my($html, $title) = ServRole::run($env);

    #my($html, $title) = ServRole::run($fig, $cgi, $sap, $user_name, $my_url, "<input type='hidden' name='page' value='SV_ServRole'>", $self);
