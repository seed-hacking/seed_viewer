package SeedViewer::WebPage::MetagenomeSequence;

# $Id: MetagenomeSequence.pm,v 1.10 2008-09-03 20:48:26 parrello Exp $
...
sub output {
  ...
  $html .= $cgi->scrolling_list( -name    => 'dataset',
				 ...
				 -multiple => 'true', );
  $html .= "</td></tr>";
  $html .= "<tr><th>Number of matches per dataset: </th><td>";
  $html .= $cgi->popup_menu( -name => 'limit', -default => $limit,
			     -values => [ 5, 10, 20, 30, 40, 50 ]);
  $html .= $limithelp->output."</td></tr>";
  ...
  foreach my $d (@datasets) {
	...
	my $onclick;
	if ($m->[0] =~ /^fig\|/) {
	  $link = "window.top.location='?page=Annotation&feature=".$m->[0]."';"; 
	  $onclick = "window.top.location='".
	    "?page=ToolResult&tool=bl2seqx&peg=".$m->[0]."&seq_id=".$seq_id."&seq=".$seq_raw."';";
	}
