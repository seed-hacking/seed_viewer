package SeedViewer::WebPage::SV_ServSubsysRow;
...
sub page_content
{
    ...
    my $env = {fig => $fig,
           ...
           user => $user_name,
           url => $my_url,
           hidden_form_var => "<input type='hidden' name='page' value='SV_ServSubsysRow'>",
    ...
    my($html, $title) = ServSubsysRow::run($env);

    #my($html, $title) = ServSubsysRow::run($fig, $cgi, $sap, $user_name, $my_url, "<input type='hidden' name='page' value='SV_ServSubsysRow'>", $self);
